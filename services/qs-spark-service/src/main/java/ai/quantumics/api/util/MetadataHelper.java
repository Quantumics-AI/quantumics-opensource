...
public class MetadataHelper {
  ...
  @Autowired private EngFlowMetaDataService engFlowMetaDataService;
  ...
  public void updateFileMetadata(
    ...
    public EngFlowMetaDataAwsRef getLatestCompletedEngFlowsInfo(String flowName) {
      List<EngFlowMetaDataAwsRef> completedFlows =
          engFlowMetaDataService.getFlowsByAthenaTableName(flowName);
      // Pick the latest Run Job record from this list...

      if (completedFlows != null && !completedFlows.isEmpty()) {
        Collections.sort(completedFlows, new Comparator<EngFlowMetaDataAwsRef>() {
          public int compare(EngFlowMetaDataAwsRef o1, EngFlowMetaDataAwsRef o2) {
            return o2.getCreatedDate().compareTo(o1.getCreatedDate());
          };
        });

        // Flow Jobs are now in Descending order of creation date. Pick the latest one now..
        return completedFlows.get(0);
      }
      return null;
    }
