...
public interface FileMetaDataAwsRepository extends JpaRepository<FileMetaDataAwsRef, Long> {
  ...
  @Modifying
  @Query(
      "select new ai.quantumics.api.user.req.PreparedDataset(re.athenaTable,re.tablePartition, re.fileType, re.fileMetaData, re.createdDate, re.createdBy, fi.fileId, fi.fileName, fl.folderId, fl.folderName,  fl.projectId) from  FileMetaDataAwsRef re "
