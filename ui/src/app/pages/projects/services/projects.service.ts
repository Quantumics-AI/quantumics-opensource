...
export class ProjectsService {
  ...
  getProjects(projectParam: ProjectParam): Observable<any> {
    return this.http.get(`/QuantumSparkServiceAPI/api/v1/projects/users/${projectParam.userId}`);
  }
  ...
  saveProject(projectId: number, projectName: string, projectDescription: string, userId: number): Observable<any> {
    const d = {
  ...
  getProjectAuditing(projectId: number, userId: number): Observable<any> {
    return this.http.get(`/QuantumSparkServiceAPI/api/v1/auditevents/${projectId}/${userId}`);
  }
