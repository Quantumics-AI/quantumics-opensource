...
class QueryRecentResource(BaseResource):
    ...
    def get(self):
        ...
        projectId = redis_connection.get("{}-project".format(self.current_user.name))
        userId = redis_connection.get("{}-userId".format(self.current_user.name))

        term = "%{}-{}%".format(userId, projectId)
        results = models.Query.query.filter(models.Query.name.ilike(term))

        result = QuerySerializer(
            results, with_last_modified_by=False, with_user=False
        ).serialize()

        for item in result:
            item['name'] = item['name'].replace('{}-{}-'.format(userId, projectId), '')
...
class BaseQueryListResource(BaseResource):
    def get_queries(self, search_term):
        ...
        if search_term:
            ...
            #results = models.Query.all_queries_project( self.current_user.group_ids, self.current_user.id, include_drafts=True,user_name=self.current_user.name)
            # results= models.Query.query.filter(redis_connection.get("{}-project".format(self.current_user.name)).contains(models.Query.name))
            # results = models.Query.search(
            #     redis_connection.get("{}-project".format(self.current_user.name)),
            #     self.current_user.group_ids,
            #     self.current_user.id,
            #     include_drafts=True,
            #     multi_byte_search=current_org.get_setting("multi_byte_search_enabled"),
        ...
        # return filter_by_name_partial(results,models.Query.name,self.current_user.name)
        print(type(results))
        return results
    
    def get_queries_by_name(self, search_term):
                
        results = models.Query.query.filter(models.Query.name.ilike(search_term))
    ...
    def get(self):
        ...
        term = "%{}-{}%".format(userId, project_id)
        results = self.get_queries_by_name(term)
        ...
        results = [x for x in response['results'] if '{}-{}'.format(userId, project_id) in x['name']]

        # replace append project from query name
        for item in results:
            item['name'] = item['name'].replace("{}-{}-".format(userId, project_id), "")
...
class QueryListResource(BaseQueryListResource):
    ...
    def post(self):
        ...
        query_def["name"] =  "{}-{}-{}".format(userId, project_id, query_def.get('name'))
        ...
        query.name = query.name.replace("{}-{}-".format(userId, project_id), "")
...
class QueryResource(BaseResource):
    ...
    def post(self, query_id):
        ...
        if not '{}-{}'.format(userId, project_id) in query_def["name"]:
            query_def["name"] =  "{}-{}-{}".format(userId, project_id, query_def.get('name'))            
    ...
    def get(self, query_id):
        ...
        # check userId and projectId is valid.
        if not '{}-{}'.format(userId, project_id) in result['name']:
            return []

        result['name'] = result['name'].replace("{}-{}-".format(userId, project_id), "")
...
class QueryFavoriteListResource(BaseResource):
    def get(self):
        ...
        search_term = "%{}_{}%".format(userId, projectId)
