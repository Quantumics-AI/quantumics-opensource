...
class QueryResultListResource(BaseResource):
    ...
    def post(self):
        ...
        try:
            for item in result:
                splitedItems = item.split('_')
                last_item_userId = splitedItems[len(splitedItems) - 1]

                if re.search('^[0-9]+$', last_item_userId) and not (userId == last_item_userId):
...
class QueryResultResource(BaseResource):
    ...
    def post(self, query_id):
        ...
        userId = redis_connection.get("{}-userId".format(self.current_user.name))
        query = query.parameterized.text
        result = query.split('.')

        try:
            if userId: # check if userId is present or not. ex - userId not present in published url
                for item in result:
                    splitedItems = item.split('_')
                    last_item_userId = splitedItems[len(splitedItems) - 1]
        ...
        if has_access(
            ...
            if 'query_result' in query_result and query_result['query_result']['data']:
                for item in query_result['query_result']['data']['columns']:
                    if item['name'] == 'partition_0':
                        # remove athena defaule added partion column from result.
                        query_result['query_result']['data']['columns'].remove(item)
    ...
    def get(self, query_id=None, query_result_id=None, filetype="json"):
        ...
        if query_result.data:
            for item in query_result.data['columns']:
                if item['name'] == 'partition_0':
                    # remove athena defaule added partion column from result.
                    query_result.data['columns'].remove(item)
