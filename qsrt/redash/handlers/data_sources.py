...
class DataSourceSchemaResource(BaseResource):
    def get(self, data_source_id):
        ...
        try:
            #modified for db filtering added username param
            response["schema"] = data_source.get_schema(self.current_user.name,refresh)
        except NotSupported:
            response["error"] = {
                "code": 1,
