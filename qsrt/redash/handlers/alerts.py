...
class AlertListResource(BaseResource):
    def post(self):
        ...
        alert = models.Alert(
            name=req["name"]+"@"+redis_connection.get("{}-project".format(self.current_user.name)),
            query_rel=query,
    ...
    def get(self):
        ...
        return [
            ...
            for alert in models.Alert.all_alerts_project(group_ids=self.current_user.group_ids, user_name=self.current_user.name)
        ]
