...
class DashboardListResource(BaseResource):
    ...
    def get(self):
        ...
        if search_term:
            ...
            # )
            results = models.Dashboard.all_dashboards_project(self.current_org, self.current_user.group_ids, self.current_user.id,self.current_user.name)
    ...
    def post(self):
        ...
        dashboard = models.Dashboard(
            name=dashboard_properties["name"]+"_"+redis_connection.get("{}-project".format(self.current_user.name)),
...
class DashboardResource(BaseResource):
    ...
    def get(self, dashboard_slug=None):
        ...
        for item in response['widgets']:
            if 'visualization' in item:
                query_name = item['visualization']['query']['name'].replace('{}-{}-'.format(userId, projectId), "")
                item['visualization']['query']['name'] = query_name
    ...
    def post(self, dashboard_slug):
        ...
        # out behind.
        # if "name" in updates and updates["name"] != dashboard.name:
        #     updates["name"] = updates["name"]+"_"+redis_connection.get("{}-project".format(self.current_user.name))
...
class PublicDashboardResource(BaseResource):
    ...
    def get(self, token):
        ...
        result = serializers.public_dashboard(dashboard)

        for item in result['widgets']:
            query_name = item['visualization']['query']['name']
            str_len = query_name.find('-')
            item['visualization']['query']['name'] = query_name[str_len + 1 : len(query_name)]
...
class DashboardFavoriteListResource(BaseResource):
    def get(self):
        ...
        for item in response['results']:
            if ('_{}'.format(projectId) in item['slug']):
                results.append(item)
