...
class DataSource(BelongsToOrgMixin, db.Model):
    ...
    def get_schema(self, username, refresh=False):
        # athena specific logic
        if self.type == "athena":
            refresh = True
            cache = None
            if not refresh:
            ...
            if cache is None:
                #dbname retrival from cache
                dbname = redis_connection.get("{}-project".format(username))
                query_runner = self.query_runner
                #inclusion of dbname parameter 
                schema = query_runner.get_schema(dbname,get_stats=refresh)
...
class Query(ChangeTrackingMixin, TimestampMixin, BelongsToOrgMixin, db.Model):
    ...
    @classmethod
    def all_queries_project(
        cls, group_ids, user_id=None, include_drafts=False, include_archived=False, user_name=None
    ):
        query_ids = (
            ...
            .filter(Query.name.contains(redis_connection.get("{}-project".format(user_name))))
...
class Alert(TimestampMixin, BelongsToOrgMixin, db.Model):
    ...
    @classmethod
    def all_alerts_project(cls, group_ids,user_name):
        return (
            cls.query.options(joinedload(Alert.user), joinedload(Alert.query_rel))
            .join(Query)
            .join(
            ...
            .filter(DataSourceGroup.group_id.in_(group_ids))
            .filter(Alert.name.contains(redis_connection.get("{}-project".format(user_name))))
...
class Dashboard(ChangeTrackingMixin, TimestampMixin, BelongsToOrgMixin, db.Model):
    ...
    @classmethod
    def all_dashboards_project(cls, org, group_ids, user_id, user_name):
        query = (
            Dashboard.query.options(
                subqueryload(Dashboard.user).load_only("_profile_image_url", "name")
            )
            ...
            .filter(
                ...
                Dashboard.name.contains(redis_connection.get("{}-project".format(user_name))),
                Dashboard.org == org,
            )
            .distinct()
        )
