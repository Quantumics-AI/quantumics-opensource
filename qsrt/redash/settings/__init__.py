...
SQLALCHEMY_MAX_OVERFLOW = os.environ.get("SQLALCHEMY_MAX_OVERFLOW", 200)
...
CONTENT_SECURITY_POLICY = os.environ.get(
    ...
    "default-src 'self' *; style-src 'self' 'unsafe-inline'; script-src 'self' 'unsafe-eval'; font-src 'self' data:; img-src 'self' http: https: data:; object-src 'none'; frame-src redash.io;",
)
